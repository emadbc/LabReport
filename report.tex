...
\end{figure}  
R100KEFST, is the instance with fractional weight, Euclidean distance and it is obtained by full Steiner tree generation for the classical Euclidean Steiner tree problem in the plane[7]. TSPFST, PUC, and LIN are the instances from \emph{SteinLib}[8] testsets. These instances all have integer weights. TSPFST is a rectilinear graph with L1 distances.  PUC contains 3 classes of the instance, hypercubes,  code covering and bipartite graphs,  and they are artificial instances(i.e. there are not derived from real world problem)[9]. The LIN instance is derived from VLSI grid graphs and it has L1 distances[10].  

\begin{figure}[h]
\centering
\includegraphics[scale=0.3]{wrpRuntime.png}
\includegraphics[scale=0.3]{wrpTerminals.png}
\caption{Running time of WRP instance in millisecond}
\end{figure}
